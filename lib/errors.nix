{lib}: let
  ...
  formatPos = pos:
    ...
    in
      ...
      then null
      else "${toString file}:${toString line}${
        if column == null
        then ""
  ...
  in
    ...
    then builtins.length aChars
    else let
      rows = lib.foldl' step initial (lib.range 0 (builtins.length aChars - 1));
    in
      builtins.elemAt
  ...
  in
    ...
    then []
      ...
      limit =
        if builtins.stringLength value <= 4
        then 1
        else 2;
  ...
  formatLocation = pos: let
    rendered = formatPos pos;
  in
    if rendered == null
    then ""
    else "\n  --> ${rendered}";
